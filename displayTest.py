...
'''
This method compares two clients' message numbers to determine if they are out of sync
'''
def checkMsgNum(checkNumber, msgNumber, sender):
    ...
    if line[1] == "":
        ...
        #diff = int(currentNum) - int(line[1])
        tempNum = line[1]
    #print("difference = " + int(currentNum) - int(tempNum))
    '''
    #print("tempNum = " + tempNum)
    print("checkNum = " + str(checkNumber))
    print( "MSGnumber = "+ str(msgNumber))
    if int(msgNumber) - int(checkNumber) != 1:
        print("Clients are out of sync!")
        #addMessage(userName + "#" + "0" + "#" + "A message has been lost! Attempting to re-sync with client...")
        fileName = userName + "#" + sender + ".txt"

        if os.path.exists(fileName):
            txtFile = open(fileName, "a")
            txtFile.write(sender + "#" + "0" + "#" + "A message has been lost! Attempting to re-sync with client...\n")
            txtFile.close()
            addMessage(sender + "#" + "0" + "#" + "A message has been lost! Attempting to re-sync with client...\n")
            sendReset(sender)
        #msg = "CHAT|" + sender + "|" + msgNum + "|" + checkSumVal + "|" + data
        #bytesToSend = str.encode(msg)
        #UDPClientSocket.sendto(bytesToSend, serverAddressPort)
        
''' 
This method sends a command message to the other client to re-sync the message number.
'''
def sendReset(sender):
    msg = "RESET|" + sender
    bytesToSend = str.encode(msg)
    UDPClientSocket.sendto(bytesToSend, serverAddressPort)
...
''' 
This method syncs the two clients together by resetting the msgNum to 0.
'''
def resetMsgNum(sender):
    fileName = userName + "#" + sender + ".txt"

    if os.path.exists(fileName):
        txtFile = open(fileName, "a")
        txtFile.write(sender + "#" + "0" + "#" + "A message to the other client has been lost. Syncing clients...\n")
        txtFile.close()
        addMessage(sender + "#" + "0" + "#" + "A message to the other client has been lost. Syncing clients...\n")
    #msg = "CHAT|" + sender + "|" + "0" + "|" + "0" + "|" + "The clients are now synced.\n"
    #bytesToSend = str.encode(msg)
    #UDPClientSocket.sendto(bytesToSend, serverAddressPort)
    #msg = "CHAT|" + receiver + "|" + msgNum + "|" + checkSumVal + "|" + data
...
def receivePackets():
    ...
    while (prog == "running"):
        ...
        if command == "ADD":
            ...
            #checkMsgNum(check, sender)
            f.close()
            #print("msgNum =" + msgNum)
            checkMsgNum(checkNum, msgNum, sender)
