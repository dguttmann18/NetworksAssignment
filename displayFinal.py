...
''' 
This method sends a command message to the other client to re-sync the message number. (Not Implemented)
'''
...
''' 
This method syncs the two clients together by resetting the msgNum to 0. (Not Implemented)
'''
def resetMsgNum(sender):
    ...
    if os.path.exists(fileName):
        txtFile = open(fileName, "a")
        txtFile.write(sender + "#" + "0" + "#" + "A message to the other client has been lost.\n")
        txtFile.close()
...
def testLoss():
    global userName, serverAddressPort, serverIPAddress
    receiver = lblReceiver.text()
    data = receiver + "#" + "0" + "#" + "This is a test to simulate loss of packets.\n"
    tempNum = int(getNextMessageNumber(receiver)) + 1
    msgNum = str(tempNum)
    checkSumVal = getCheckSumValue(data)
